...
function loadPlaylist() {
    $.getJSON("http://api.yetanother.pw:25404/playlist", function(data) {
        ...
        if (data.length == 1) {
            $('<li class="list-group-item"><span class="badge">' + '</span>' + "Nothing in the queue now." + '</li>').appendTo('.list-group');
        } else {
            for (var i = 0; i < data.length; i++) {
                $('<li class="list-group-item"><span class="badge">' + data[i].added_by + '</span>' + data[i].name + '</li>').appendTo('.list-group');
            }
        }
    });
}
